package com.example.beeli.ui.Iklan.FormIklan.Jasa;
...
import com.example.beeli.databinding.FormTambahIklanJasaBinding;
...
public class FormIklanJasaPekerjaan extends AppCompatActivity {
    FormTambahIklanJasaBinding binding;
    private String tipe, gajiDari, gajiSampai, judul_iklan, deskripsi;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        binding = FormTambahIklanJasaBinding.inflate(getLayoutInflater());
    ...
    private void view() {
        ...
        binding.kategori.setText("Jasa & Lowongan / Cari Pekerjaan");
        binding.ubah.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(FormIklanJasaPekerjaan.this, PilihKategoriIklan.class);
                    PilihKategoriIklan.menuKategoriIklan.finish();
                    JasaIklan.subJasa.finish();
        ...
        ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(this, R.array.tipeKerja, android.R.layout.simple_spinner_item);
        adapter.setDropDownViewResource(androidx.appcompat.R.layout.support_simple_spinner_dropdown_item);
        binding.tipeJasa.setAdapter(adapter);
        binding.tipeJasa.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                if (position != 0) {
                    tipe = binding.tipeJasa.getItemAtPosition(position).toString();
                }
            }
        ...
        binding.linearLayoutGajiDari.setVisibility(View.GONE);
        binding.linearLayoutGajiSampai.setVisibility(View.GONE);
    ...
    private void checkInput() {
        //        get the variabels
        gajiDari = binding.gajiDariJasa.getText().toString();
        gajiSampai = binding.gajiSampaiJasa.getText().toString();
        judul_iklan = binding.judulIklanJasa.getText().toString();
        deskripsi = binding.deskripsiJasa.getText().toString();

        boolean b = true;

        if (tipe.isEmpty()) {
            Toast.makeText(this, "Mohon memilih Tipe Jasa", Toast.LENGTH_SHORT).show();
            b = false;
        } else if (gajiDari.isEmpty()) {
            Toast.makeText(this, "Mohon mengisi Gaji minimal", Toast.LENGTH_SHORT).show();
            b = false;
        } else if (gajiSampai.isEmpty()) {
            Toast.makeText(this, "Mohon mengisi Gaji maximal", Toast.LENGTH_SHORT).show();
            b = false;
        } else if (judul_iklan.isEmpty()) {
            Toast.makeText(this, "Mohon mengisi Judul Iklan", Toast.LENGTH_SHORT).show();
            binding.judulIklanJasa.setError("Tidak boleh kosong");
            b = false;
        } else if (deskripsi.isEmpty()) {
            Toast.makeText(this, "Mohon mengisi Deskripsi Barang", Toast.LENGTH_SHORT).show();
            binding.deskripsiJasa.setError("Tidak boleh kosong");
            b = false;
...
//            if user want to continue
    ...
    private void post() {
        ...
        builder.addFormDataPart("gaji_dari", null);
        builder.addFormDataPart("gaji_sampai", null);
        ...
        ApiService.getToken(new ApiService.TokenCallback() {
            ...
            public void onTokenReceived(String token) {
                ApiService.endpoint(token).postIJasa("caripekerjaan", requestBody).enqueue(new Callback<ResponseBody>() {
                    @Override
                    public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                        if (response.isSuccessful()) {
                            ...
                            binding.btnSimpan.setEnabled(true);
                            binding.btnSimpan.setText("Pasang Iklan Sekarang");
                            Log.d("JasaPekerjaanIklan", "onResponse: " + response.code());
                            Log.d("JasaPekerjaanIklan", "onResponse: " + response.message());
                            Toast.makeText(FormIklanJasaPekerjaan.this, response.message(), Toast.LENGTH_SHORT).show();
                        }
                    }
                    ...
                    public void onFailure(Call<ResponseBody> call, Throwable throwable) {
                        ...
                        binding.btnSimpan.setEnabled(true);
                        binding.btnSimpan.setText("Pasang Iklan Sekarang");
                        Log.d("JasaPekerjaanIklan", "onResponse Failure: " + throwable.getMessage());
                        Toast.makeText(FormIklanJasaPekerjaan.this, throwable.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
            }
            ...
            public void onTokenError() {
                 Helper.disableDialog();
                Toast.makeText(FormIklanJasaPekerjaan.this, "Failed to get token", Toast.LENGTH_SHORT).show();
            }
        });

package com.example.beeli.ui.Iklan.FormIklan.Mobil;
...
import com.example.beeli.databinding.FormTambahIklanMobilBinding;
...
public class FormIklanMobilBekas extends AppCompatActivity {
    FormTambahIklanMobilBinding binding;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        binding = FormTambahIklanMobilBinding.inflate(getLayoutInflater());
    ...
    private void view() {
        ...
        binding.kategori.setText("Mobil / Mobil Bekas");
        binding.ubah.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(FormIklanMobilBekas.this, PilihKategoriIklan.class);
                    PilihKategoriIklan.menuKategoriIklan.finish();
                    MobilIklan.subMobil.finish();
        ...
        binding.tahunMobil.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                if (position != 0){
                tahun = binding.tahunMobil.getItemAtPosition(position).toString();
                }
            }
    ...
    private void checkInput() {
        //        get the variabels
        merk = binding.merkMobil.getText().toString();
        judul_iklan = binding.judulIklanMobil.getText().toString();

        warna = binding.warnaMobil.getText().toString();
        deskripsi = binding.deskripsiMobil.getText().toString();
        harga = binding.hargaMobil.getText().toString();

        boolean b = true;

        if (merk.isEmpty()) {
            Toast.makeText(this, "Mohon mengisi Merk Barang", Toast.LENGTH_SHORT).show();
            binding.merkMobil.setError("Tidak boleh kosong");
            b = false;
        } else if (judul_iklan.isEmpty()) {
            Toast.makeText(this, "Mohon mengisi Judul Iklan", Toast.LENGTH_SHORT).show();
            binding.judulIklanMobil.setError("Tidak boleh kosong");
            b = false;
        } else if (tahun.isEmpty()) {
            Toast.makeText(this, "Mohon memilih Tahun Mobil", Toast.LENGTH_SHORT).show();
            binding.judulIklanMobil.setError("Tidak boleh kosong");
            b = false;
            ...
            binding.deskripsiMobil.setError("Tidak boleh kosong");
            b = false;
        } else if (harga.isEmpty()) {
            Toast.makeText(this, "Mohon mengisi Harga Barang", Toast.LENGTH_SHORT).show();
            binding.hargaMobil.setError("Tidak boleh kosong");
            b = false;
...
//            if user want to continue
    ...
    private void post() {
        ...
        builder.addFormDataPart("tahun", tahun);
        builder.addFormDataPart("tipe_bahan_bakar", tipeBahanBakar);
        builder.addFormDataPart("warna", warna);
        ...
        ApiService.getToken(new ApiService.TokenCallback() {
            ...
            public void onTokenReceived(String token) {
                ApiService.endpoint(token).postIMobil("bekas", requestBody).enqueue(new Callback<ResponseBody>() {
                    @Override
                    public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                        if (response.isSuccessful()) {
                            ...
                            binding.btnSimpan.setEnabled(true);
                            binding.btnSimpan.setText("Pasang Iklan Sekarang");
                            Log.d("MobilBekasIklan", "onResponse: " + response.code());
                            Log.d("MobilBekasIklan", "onResponse: " + response.message());
                            Toast.makeText(FormIklanMobilBekas.this, response.message(), Toast.LENGTH_SHORT).show();
                        }
                    }
                    ...
                    public void onFailure(Call<ResponseBody> call, Throwable throwable) {
                        ...
                        binding.btnSimpan.setEnabled(true);
                        binding.btnSimpan.setText("Pasang Iklan Sekarang");
                        Log.d("MobilBekasIklan", "onResponse Failure: " + throwable.getMessage());
                        Toast.makeText(FormIklanMobilBekas.this, throwable.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
            }
            ...
            public void onTokenError() {
                 Helper.disableDialog();
                Toast.makeText(FormIklanMobilBekas.this, "Failed to get token", Toast.LENGTH_SHORT).show();
            }
        });
    }
